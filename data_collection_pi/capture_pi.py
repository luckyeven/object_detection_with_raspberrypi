...
save_path = "../images/num01"      # Save images to directory
...
def get_filepath():
    """
    Returns the next avaiable full path to image file
    """
    ...
    with PiCamera() as camera:
        ...
        # Container for out frames
        raw_capture = PiRGBArray(camera, size = (res_width, res_height))

        # Initial countdown timestamp
        countdown_timestamp = cv2.getTickcount()

        # Continuosusly capture frames
        for frame in camera.capture_continuous(raw_capture,format='bgr', use_video_port = True):
            # Get timestamp for calculationg actual framerate
            timestamp = cv2.getTickCount()

            # GetNumpy array that represents the image
            img = frame.array
            ...
            # Clear the stream to prepare for next fram
            raw_capture.truncate(0)

            # Calculate framrate
            frame_time = (cv2.getTickCount() - timestampe) / cv2.getTickFrequency()
            fps = 1 / frame_time
